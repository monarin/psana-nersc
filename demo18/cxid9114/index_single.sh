...
if [ $# -eq 0 ]; then
    echo "Usage: ./index_single.sh EXP RUN_NO TRIAL_NO CMDMODE(none/pythonprof/strace/debug) MAX_EVT"
    exit 0
fi
...
cctbx_args="input.experiment=${EXP} input.run_num=${RUN} output.logging_dir=${OUT_DIR} output.output_dir=${OUT_DIR} format.cbf.invalid_pixel_mask=${IN_DIR}/mask_ld91.pickle input.reference_geometry=/tmp/geom_ld91.json /tmp/process_batch.phil dump_indexed=False output.tmp_output_dir=${OUT_DIR} input.xtc_dir=${DATA_DIR}"
