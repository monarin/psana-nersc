...
ds = DataSource(exp='tmolv9418', run=175, dir=xtc_dir, batch_size=batch_size, max_events=max_events)
...
#         continue
        ...
        for ndigi,(digitizer,fexdata) in enumerate(fex.items()):
            for nfex,(channel,fexchan) in enumerate(fexdata.items()):
                ...
                for ipeak, peak in enumerate(peaks):
                    ...
                    stimes[st:en] = np.arange(startpos[ipeak], 
                            startpos[ipeak]+peaks_sizes[ipeak]) * sample_period
    ...
    if nevt % batch_size == 0:
        en_batch = time.time()
        print(f'RANK:{rank} processed {nevt} events rate:{(batch_size/(en_batch-st_batch))*1e-3:.2f}kHz', flush=True)
...
if rank == 0:
    ...
    print(f'TOTAL TIME:{t1-t0:.2f}s #EB:{n_eb_nodes} #SRV:{n_srv_nodes} smd_batch_size:{smd_batch_size}')
