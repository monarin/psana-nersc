...
def test_standard():
    ...
    if my_data[0] >= 0:
        comm.Reduce([my_data, MPI.DOUBLE], [totals, MPI.DOUBLE], op=MPI.SUM, root=0)
...
def test_psana2_hang():
    ...
    for i, evt in enumerate(run.events()):
        ...
        print(
            f"bdrank:{bd_rank} i:{i} evt:{evt.timestamp} hsd[0]:{hsd_calib[0][0]} sum:{hsd_sum[0][0]}",
            flush=True,
        )
        ...
        if i % 3 == 0:
            ...
            bd_comm.Reduce(
                [hsd_sum, MPI.DOUBLE], [totals, MPI.DOUBLE], op=MPI.SUM, root=0
            )
...
def test_psana2_pass():
    ...
    for i_step, step in enumerate(run.steps()):
        ...
        print(
            f"bdrank:{bd_rank} step:{i_step} #events:{cn_events} hsd[0]:{hsd_calib[0][0]} sum:{hsd_sum[0][0]}",
            flush=True,
        )
        ...
        bd_comm.Reduce([hsd_sum, MPI.DOUBLE], [totals, MPI.DOUBLE], op=MPI.SUM, root=0)

        if bd_rank == 0:
            print(
                f"  bdrank:{bd_rank} step:{i_step} totals: {totals[0][0]}", flush=True
            )
