...
with open(filename, "r") as f:
    ...
    if show_plot:
        plt.hist(data)
        plt.title(
            f"Histogram of {label[view_task_id]} #cn:{cn[view_task_id]} total:{sum_t[view_task_id]*1e3:.2f} {unit}"
        )
        plt.xlabel(unit)
        plt.show()
