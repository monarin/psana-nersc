...
def PhotonSpectrumGauss(proj, hw):
    ...
    p, q = curve_fit(
        ...
        x,
        y,
        p0=[proj[x_max], x_max, hw / 2, np.mean(proj[0 : min(20, inda)])],
    )
...
def plot2d(
    ...
    if log10:
        ...
        im = plt.imshow(
            mat,
            extent=[bins1.min(), bins1.max(), bins2.min(), bins2.max()],
            aspect="auto",
            cmap=cmap,
            norm=colors.SymLogNorm(
                linthresh=1, linscale=1, vmin=vmin, vmax=vmax, clip=False
            ),
        )
    else:
        im = plt.imshow(
...
if __name__ == "__main__":
    ...
    if rank == 0:
        print(
            f"Processing time:{t2-t1:.2f} #evt:{mevt} rate:{(mevt/(t2-t1))*1e-3:.2f}kHz"
        )
