...
def test_fakestep():
    ...
    xtc_dir = "/cds/home/m/monarin/psana-nersc/psana2/data/fakesteps"
    ...
    if rank == 0:
        ...
        for i in range(size):
            if result[i]["evt_per_step"]:
                sum_events_per_step += np.asarray(
                    result[i]["evt_per_step"], dtype=np.int32
                )
            sum_events += result[i]["n_events"]
            n_steps = np.max([n_steps, result[i]["n_steps"]])

        print(
            f"sum_events_per_step:{sum_events_per_step} sum_events:{sum_events} sum_steps:{n_steps}"
        )
