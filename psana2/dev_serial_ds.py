...
if __name__ == "__main__":
    ...
    smd_files = np.asarray(
        [
            "/reg/neh/home/monarin/data/smalldata/data-r0001-s%s.smd.xtc2"
            % str(i).zfill(2)
            for i in range(nfiles)
        ]
    )
    xtc_files = np.asarray(
        [
            "/reg/neh/home/monarin/data/data-r0001-s%s.xtc2" % str(i).zfill(2)
            for i in range(nfiles)
        ]
    )
    ...
    if len(delta_t_sml) > 0:
        print(
            "#fast_evts: %d mean (ms) %6.4f min %6.4f max %6.4f std %6.4f"
            % (
                len(delta_t_sml),
                np.mean(delta_t_sml),
                np.min(delta_t_sml),
    ...
    if len(delta_t_big) > 0:
        print(
            "Batch read (s) mean: %6.4f min: %6.4f max: %6.4f std: %6.4f"
            % (
                np.mean(delta_t_big),
                np.min(delta_t_big),
                np.max(delta_t_big),
    ...
    print(
        "Total Elapsed (s): %6.2f Rate (kHz): %6.2f Bandwidth (MB/s): %6.2f"
        % (
            total_t,
            max_events / (total_t * 1000),
            (737 * nfiles * max_events) / (total_t * 1000000),
        )
    )
