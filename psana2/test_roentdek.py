...
def proc_data(**kwargs):
    XTCDIR = kwargs.get(
        "xtcdir", "/cds/home/m/monarin/psana-nersc/psana2/dgrampy/amox27716"
    )
    ...
    if TESTMODE == 4:
        smd_batch_size = 1000
        smd = ds.smalldata(
            filename="/cds/data/drpsrcf/users/monarin/amox27716/out/mysmallh5.h5",
        ...
        if TESTMODE in (2, 3, 4):
            ...
            for i_det in range(DETECTORS):
                for i_chan in range(NUMCHS):
                    ...
                    if VERBOSE:
                        print(
                            f"  det: {i_det} chan:{i_chan} #peaks:{len(result_peaks)}"
                        )
            ...
            if np.all(n_result_peaks == 0):
                cn_no_peaks_events += 1
        ...
        if TESTMODE in (3, 4) and np.sum(n_result_peaks) > 0:
            for i_det in range(DETECTORS):
                ...
                if ROENTDEK == "dldproc":
                    ...
                    for i, (x, y, r, t) in enumerate(
                        proc.xyrt_list(i_ev, nhits_fex[st:en], pktsec_fex[st:en, :])
                    ):
                        ...
                        if VERBOSE:
                            print(
                                "    hit:%2d x:%7.3f y:%7.3f t:%10.5g r:%7.3f"
                                % (i, x, y, t, r)
                            )
                else:
                    # Xiang's peakfinding
                    HF.FindHits(
                        pktsec_fex[4, : nhits_fex[4]],
                ...
                if PLOT:
                    stats.fill_data(nhits_fex[st:en], pktsec_fex[st:en, :])
    ...
    if PLOT:
        draw_plots(
            stats,
            prefix=OFPREFIX + EXP + f"-r{str(RUN).zfill(4)}",
            do_save=True,
            hwin_x0y0=(0, 10),
        )
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        print(
            f"EVENTS: {nev} MODE: {kwargs['testmode']} #EB: {n_ebs} #SRV: {n_srvs} ELAPSED TIME: {t1-t0:.2f}s TOTALRATE: {(nev/(t1-t0))*1e-3:.4f}kHz"
        )
