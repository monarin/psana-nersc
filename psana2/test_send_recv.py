...
if rank == 0:
    ...
    print(
        f"send timing #sends: {len(send_timings)} data: {data_nbytes/1e6:.2f}MB rate avg: {np.mean(rates):.2f}GB/s max: {np.max(rates):.2f}GB/s min: {np.min(rates):.2f}GB/s total={total_rate}GB/s"
    )
