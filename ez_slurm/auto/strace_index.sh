...
#run index
#python -m cProfile -s tottime /modules/cctbx_project/xfel/command_line/xtc_process.py input.experiment=${EXP} input.run_num=${RUN} output.logging_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/stdout output.output_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/out ${BASE_DIR}/discovery/target.phil

#strace -ttt -f -o $$.log cctbx.xfel.xtc_process input.experiment=${EXP} input.run_num=${RUN} output.logging_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/stdout output.output_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/out ${BASE_DIR}/discovery/target.phil

strace -ttt -f -o strace_$$.log cctbx.xfel.xtc_process input.experiment=${EXP} input.run_num=${RUN} output.logging_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/$$/stdout output.output_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/$$/out format.cbf.invalid_pixel_mask=${BASE_DIR}/calib/mask_ld91.pickle input.reference_geometry=${BASE_DIR}/discovery/geom_ld91.json ${BASE_DIR}/discovery/process_batch.phil dump_indexed=False output.tmp_output_dir=${OUT_DIR}/discovery/dials/${RUN_F}/${TRIAL_F}/$$/tmp max_events=100
