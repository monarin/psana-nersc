...
%s input.experiment=experimentname input.run_num=N input.address=address
 format.file_format=pickle input.cfg=filename
  ...
  def run(self):
    ...
    end_phil = time.time()
    print "PROFILEPHIL", start_phil, end_phil, end_phil-start_phil
    ...
    for run in ds.runs():
      ...
      if params.mp.method == "mpi" and params.mp.mpi.method == 'client_server' and size > 2:
        ...
        start_proc_rank = time.time()
        print "PROFILEPROCRANK", rank, start_proc_rank
        comm.Barrier()
        start_proc = MPI.Wtime()
        try:
          if rank == 0:
            # server process
            ...
            while True:
              ...
              end_jump = MPI.Wtime()
              print "PROFILEJUMP", start_jump, end_jump, end_jump - start_jump
              self.process_event(run, evt)
              end_integrate = MPI.Wtime()
              print "PROFILEINT", end_jump, end_integrate, end_integrate - end_jump
        except Exception, e:
          print "Error caught in main loop"
          print str(e)
        comm.Barrier()
        end_proc = MPI.Wtime()
        if rank == 0: print "PROFILEPROC", start_proc, end_proc, end_proc-start_proc
      else:
        import resource
